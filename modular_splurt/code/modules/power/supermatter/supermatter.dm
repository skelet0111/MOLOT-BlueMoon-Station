...
//	DELAMINATION B: Too few engineers, minimal explosion.
		if(0)
			...
			explosion(get_turf(src), ((explosion_power*gasmix_power_ratio)*EXPLOSION_MODIFIER_SMALL * 0.05), ((explosion_power*gasmix_power_ratio)*EXPLOSION_MODIFIER_SMALL * 0.2), ((explosion_power*gasmix_power_ratio)*EXPLOSION_MODIFIER_SMALL * 0.3), ((explosion_power*gasmix_power_ratio)*EXPLOSION_MODIFIER_SMALL * 0.5), TRUE, TRUE, ((explosion_power*gasmix_power_ratio)*EXPLOSION_MODIFIER_SMALL * 0.5))
			qdel(src)
			return
...
//	DELAMINATION D:
		if(2 to INFINITY)
			...
			explosion(get_turf(src), ((explosion_power*gasmix_power_ratio)*EXPLOSION_MODIFIER_LARGE * 0.05), ((explosion_power*gasmix_power_ratio)*EXPLOSION_MODIFIER_LARGE * 0.2), ((explosion_power*gasmix_power_ratio)*EXPLOSION_MODIFIER_LARGE * 0.3), ((explosion_power*gasmix_power_ratio)*EXPLOSION_MODIFIER_LARGE * 0.5), TRUE, TRUE, ((explosion_power*gasmix_power_ratio)*EXPLOSION_MODIFIER_LARGE * 0.5))
			qdel(src)
			return
		if(null)
			investigate_log("has delaminated, but there are only [alive_engineers] engineers! Defaulting to minimum explosion.", INVESTIGATE_SUPERMATTER)
