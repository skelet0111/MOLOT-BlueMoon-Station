	...
	for(var/M in invokers)
		if(isliving(M))
			...
			if(invocation)
				...
				playsound(get_turf(L), BLOOD_SCREAMS_PICK, 65, 1, 1)
			if(invoke_damage)
...
/obj/effect/rune/manifest/invoke(var/list/invokers)
	...
	if(choice == "Summon a Cult Ghost")
		...
		if(ghosts >= ghost_limit)
			...
			return list()
		notify_ghosts("Manifest rune invoked in [get_area(src)].", 'sound/effects/ghost2.ogg', source = src, notify_suiciders = TRUE)
