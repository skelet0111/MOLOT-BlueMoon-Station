...
/obj/item/circuit_component/bci_core/input_received(datum/port/input/port)
	...
	if (COMPONENT_TRIGGERED_BY(send_message_signal, port))
		...
		to_chat(resolved_owner, "<i>Вы слышите странный механический голос в голове...</i> \"[span_robot("[html_encode(sent_message)]")]\"")
...
/obj/item/circuit_component/bci_core/proc/on_examine(datum/source, mob/mob, list/examine_text)
	...
	if (isobserver(mob))
		examine_text += span_notice("[source.ru_who(capitalized = TRUE)] [source.p_have()] <a href='?src=[REF(src)];open_bci=1'>\a [parent] implanted in [source.ru_na()]</a>.")
