...
/obj/item/drone_shell/Initialize(mapload)
	...
	if(A)
		notify_ghosts("A drone shell has been created in \the [A.name].", source = src, action=NOTIFY_ATTACK, flashwindow = FALSE, ignore_key = POLL_IGNORE_DRONE, notify_suiciders = TRUE)
	GLOB.poi_list |= src
