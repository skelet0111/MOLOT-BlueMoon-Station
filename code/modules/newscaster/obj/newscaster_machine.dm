...
/obj/machinery/newscaster/wrench_act(mob/user, obj/item/I)
	...
	if(stat & BROKEN)
		...
		var/obj/item/wallframe/newscaster/frame = new(loc)
...
/obj/machinery/newscaster/ui_data(mob/user)
	...
	switch(screen)
		...
		if(NEWSCASTER_JOBS)
			var/list/jobs = list()
			data["jobs"] = jobs
			for(var/cat in list("law", "security", "engineering", "medical", "science", "service", "supply"))
				jobs[cat] = list()
			...
			for(var/datum/job/job in SSjob.occupations)
				...
				if(job.is_position_available())
					...
					if(job.title in GLOB.law_positions)
						jobs["law"] += opening_data
					if(job.title in GLOB.security_positions)
						jobs["security"] += opening_data
					if(job.title in GLOB.engineering_positions)
						jobs["engineering"] += opening_data
					if(job.title in GLOB.medical_positions)
						jobs["medical"] += opening_data
					if(job.title in GLOB.science_positions)
						jobs["science"] += opening_data
					if(job.title in GLOB.civilian_positions)
						jobs["service"] += opening_data
					if(job.title in GLOB.supply_positions)
	...
	if(photo && data["modal"] && (data["modal"]["id"] in list("create_story", "wanted_notice")))
		...
		user << browse_rsc(photo.picture, "inserted_photo_[photo.UID()].png")
	else
		data["photo"] = null
...
/obj/machinery/newscaster/proc/get_message_data(datum/feed_message/FM, mob/M)
	...
	return list(list(
		...
		publish_time_proper = STATION_TIME_TIMESTAMP("hh:mm:ss", FM.publish_time),
	...
	switch(action)
		...
		if("attach_photo")
			...
			if(ishuman(usr))
				...
				if(istype(P) && usr.transferItemToLoc(P, src))
					photo = P
				...
				var/obj/item/photo/copied_pic = new(src, selection.Copy())
				P.forceMove(copied_pic)
				photo = P
				return TRUE
...
/obj/machinery/newscaster/proc/eject_photo(mob/user)
	...
	photo = null
	P.forceMove(loc)
	if(ishuman(user) && user.put_in_active_hand(P, ignore_animation = FALSE))
...
/obj/machinery/newscaster/proc/get_scanned_user(mob/user)
	...
	if(ishuman(user))
		...
		if(istype(id))
			return list(name = "[id.registered_name] ([id.assignment])", security = allowed(list(ACCESS_SECURITY), TRUE, id.access))
