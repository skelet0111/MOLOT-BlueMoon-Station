/proc/priority_announce(text, title = "", sound, type , sender_override, has_important_message)
	...
	if(type == "Priority")
		...
		GLOB.news_network.submit_article(html_encode(text), "Капитан Объявляет (— [usr.name])", "Станционные Объявления", null)
	else if(type == "Syndicate")
		announcement += "<h1 class='alert'>Синдикат Объявляет</h1>"
		GLOB.news_network.submit_article(html_encode(text), "Синдикат Объявляет", "Станционные Объявления", null)
	else if(type == "AI")
		...
		if(!sender_override)
			if(title == "")
				GLOB.news_network.submit_article(text, "Центральное Командование Объявляет", "Станционные Объявления", null)
			else
				GLOB.news_network.submit_article(title + "<br><br>" + text, "Центральное Командование Объявляет", "Станционные Объявления", null)
