				...
				switch(href_list["field"])
					...
					if("criminal")
						if(istype(active2, /datum/data/record))
							...
							temp += "<li><a href='?src=[REF(src)];choice=Change Criminal Status;criminal2=parolled'>Paroled</a></li>"
	...
	for(var/datum/data/record/R in GLOB.data_core.security)
		if(prob(severity/10))
			switch(rand(1,8))
				...
				if(4)
					R.fields["criminal"] = pick(SEC_RECORD_STATUS_NONE, SEC_RECORD_STATUS_ARREST, SEC_RECORD_STATUS_EXECUTE, SEC_RECORD_STATUS_INCARCERATED, SEC_RECORD_STATUS_RELEASED, SEC_RECORD_STATUS_PAROLLED, SEC_RECORD_STATUS_DEMOTE, SEC_RECORD_STATUS_SEARCH, SEC_RECORD_STATUS_MONITOR, SEC_RECORD_STATUS_DISCHARGED)
				if(5)
