...
/obj/item/clockwork/clockslab/attack_self(mob/user)
	...
	if(enchant_type == TELEPORT_SPELL)
		...
		if(do_after(user, 1.5 SECONDS, target = user) && destination)
			...
			log_attack("[user] teleported to [destination] by [src]")
			deplete_spell()
		user.color = null
...
/obj/item/clockwork/clockslab/afterattack(atom/target, mob/living/user, proximity, params)
	...
	switch(enchant_type)
		if(STUN_SPELL)
			if(!isliving(target) || is_servant_of_ratvar(target) || !proximity)
				return
			...
			log_attack("[user] stunned [target] by [src]")
			deplete_spell()
		if(KNOCK_SPELL)
		...
		if(TELEPORT_SPELL)
			...
			if(do_after(user, 1.5 SECONDS, target = user))
				...
				playsound(user, 'sound/effects/phasein.ogg', 20, TRUE, SHORT_RANGE_SOUND_EXTRARANGE)
				log_attack("[user] teleported to [target] by [src]")
				deplete_spell()
			user.color = null
		if(HEAL_SPELL)
			if(!isliving(target) || !is_servant_of_ratvar(target) || !proximity)
			...
			if(ishuman(living))
				...
				if(M.health < M.maxHealth)
					M.adjustHealth(-50)
			log_attack("[user] clockslab healed [target] by [src]")
			deplete_spell()
...
/obj/item/twohanded/ratvarian_spear/throw_impact(atom/hit_atom, datum/thrownthing/throwingdatum)
	...
	if(is_servant_of_ratvar(living))
...
/obj/item/twohanded/ratvarian_spear/afterattack(atom/target, mob/user, proximity, params)
	...
	switch(enchant_type)
		if(CONFUSE_SPELL)
			...
			log_attack("[user] inflicted confusion to [target] with [src]")
			deplete_spell()
		if(DISABLE_SPELL)
			...
			log_attack("[user] point-EMP [target] with [src]")
			deplete_spell()
...
/obj/item/twohanded/clock_hammer/afterattack(atom/target, mob/user, proximity, params)
	...
	switch(enchant_type)
		if(KNOCKOFF_SPELL)
			...
			log_attack("[user] knocked-off [target] with [src]")
			deplete_spell()
		if(CRUSH_SPELL)
			if(ishuman(living))
			...
			if(isrobot(living))
				...
				RC.destroy()
			log_attack("[user] crushed [target] with [src]")
			deplete_spell()
...
/obj/item/melee/clock_sword/attack_self(mob/user)
	...
	if(enchant_type == FASTSWORD_SPELL && src == user.get_active_hand())
		ADD_TRAIT(src, TRAIT_NODROP, CLOWN_NUKE_TRAIT)
		enchant_type = CASTING_SPELL
		force = 7
		swordsman = TRUE
		log_attack("[user] sworded [target] with [src]")
...
/obj/item/melee/clock_sword/proc/reset_swordsman(mob/user)
	...
	REMOVE_TRAIT(src, TRAIT_NODROP, CLOWN_NUKE_TRAIT)
	force = initial(force)
	swordsman = FALSE
	deplete_spell()
	...
	if(enchant_type == FLASH_SPELL)
		...
		for(var/mob/living/carbon/M in oviewers(3, user))
			...
			if(M.flash_act(2, 1))
				...
				log_attack("[user] flashed [target] with [src]")
		deplete_spell()
...
/obj/item/clothing/suit/hooded/clockrobe/ui_action_click(mob/user, actiontype)
	if(actiontype == /datum/action/item_action/activate/enchant)
		if(!iscarbon(user))
			return
		var/mob/living/carbon/carbon = user
		if(carbon.wear_suit != src || !is_servant_of_ratvar(carbon))
			return
		if(enchant_type == INVIS_SPELL)
			...
			sleep(10)
			carbon.alpha = 20
			log_attack("[user] cloaked with [src]")
			addtimer(CALLBACK(src, PROC_REF(uncloak), carbon), 10 SECONDS)
		if(enchant_type == SPEED_SPELL)
			enchant_type = CASTING_SPELL
			ADD_TRAIT(src, TRAIT_NODROP, CLOWN_NUKE_TRAIT)
			carbon.add_movespeed_modifier(/datum/movespeed_modifier/reagent/stimulants)
			...
			log_attack("[user] speed boosted with [src]")
	else
		ToggleHood(user)
...
/obj/item/clothing/suit/hooded/clockrobe/proc/unspeed(mob/living/carbon/carbon)
	carbon.remove_movespeed_modifier(/datum/movespeed_modifier/reagent/stimulants)
	REMOVE_TRAIT(src, TRAIT_NODROP, CLOWN_NUKE_TRAIT)
	deplete_spell()
...
/obj/item/clothing/suit/armor/clockwork/hit_reaction(mob/living/carbon/human/owner, atom/movable/hitby, attack_text, final_block_chance, damage, attack_type)
	if(enchant_type == ABSORB_SPELL && is_servant_of_ratvar(owner))
...
/obj/item/clothing/suit/armor/clockwork/IsReflect(def_zone)
	...
	if(owner.wear_suit != src)
		return FALSE
	if(enchant_type == REFLECT_SPELL && is_servant_of_ratvar(owner))
		playsound(loc, "sparks", 100, TRUE)
		new /obj/effect/temp_visual/ratvar/sparks(get_turf(owner))
		if(reflect_uses <= 0)
...
/obj/item/clothing/suit/armor/clockwork/attack_self(mob/user)
	...
	if(enchant_type == ARMOR_SPELL)
		...
		ADD_TRAIT(src, TRAIT_NODROP, CLOWN_NUKE_TRAIT)
		enchant_type = CASTING_SPELL
		log_attack("[user] hardened with [src]")
...
/obj/item/clothing/gloves/clockwork/attack_self(mob/user)
	...
	switch(enchant_type)
		if(FASTPUNCH_SPELL)
			...
			enchant_type = CASTING_SPELL
			north_star = TRUE
			log_attack("[user] north-starred [src]")
		...
		if(FIRE_SPELL)
			...
			enchant_type = CASTING_SPELL
			fire_casting = TRUE
			log_attack("[user] fire-casted [src]")
...
/obj/item/clothing/gloves/clockwork/Touch(atom/A, proximity)
	...
	if(enchant_type == STUNHAND_SPELL && isliving(A))
		...
		log_attack("[user] stunned [living] with [src]")
		deplete_spell()
...
/obj/item/clockwork/shard/attack_self(mob/user)
	...
	if(!enchant_type)
		...
		switch(enchant_type)
			if(EMP_SPELL)
				log_attack("[user] clock EMP with [src]")
				empulse(src, 4, 6, cause="clock")
				qdel(src)
			if(TIME_SPELL)
				log_attack("[user] time stopped with [src]")
				qdel(src)
				new /obj/effect/timestop/clockwork(get_turf(user))
			if(RECONSTRUCT_SPELL)
				log_attack("[user] reconstructed with [src]")
				qdel(src)
...
/obj/item/clockwork/shard/afterattack(atom/target, mob/user, proximity, params)
	. = ..()
	if(!ishuman(target) || !is_servant_of_ratvar(user))
		return
	if(!proximity)
		return
	var/mob/living/carbon/human/human = target
...
/obj/effect/temp_visual/ratvar/reconstruct/proc/reconstruct()
	...
	for(var/atom/affected in range(4, get_turf(src)))
		if(isliving(affected))
			...
			if(!is_servant_of_ratvar(living) && !ishuman(living))
				continue
